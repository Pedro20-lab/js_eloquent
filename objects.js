...
they are both independent lists, but they share the structure that
makes up their last three elements. The original list is also still 
...
produces an array from a list. Add the helper functions prepend, 
which takes an element and a list and creates a new list 
that adds the element to the front of the input list, and nth, 
which takes a list and a number and returns the element at the 
...
function recursive_nth(list, index, node = 0) {
    if (index !== node) {
        return recursive_nth(list.rest, index, node + 1)
    } else {
        return list.value
    }
}
